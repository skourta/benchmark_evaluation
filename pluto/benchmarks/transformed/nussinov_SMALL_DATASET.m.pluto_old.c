...
/* Start of CLooG code */
if (_PB_N >= 2) {
  for (t2=max(-1,ceild(-_PB_N-29,32));t2<=floord(_PB_N-1,32);t2++) {
    lbp=max(0,t2);
    ubp=min(floord(_PB_N-1,32),floord(32*t2+_PB_N+29,32));
#pragma omp parallel for private(lbv,ubv,t5,t6,t7,t8,t9,t10)
    for (t4=lbp;t4<=ubp;t4++) {
      if ((t2 <= floord(32*t4-_PB_N+2,32)) && (t4 >= ceild(_PB_N-31,32))) {
